...
USER_SCOPE = ['me', 'users', 'categories', 'events', 'signs', 'tags', 'messages']
...
def create_access_token(username: str, scopes: list[str] = [], expires_delta: int = None):
...
def create_refresh_token(username: str, scopes: list[str] = [], expires_delta: int = None):
    if expires_delta is not None:
        expires_delta = datetime.utcnow() + expires_delta
    else:
        expires_delta = datetime.utcnow(
